...
import React, { Component, useCallback, memo, useState, useEffect } from 'react';
import {
    ScrollView,
    StyleSheet,
    Text,
...
// Access the native module for Windows-specific functionality
...
// Windows-specific component for testing Windows APIs
const WindowsSpecificComponent = () => {
    const [windowsInfo, setWindowsInfo] = useState({
        diskCapacity: 'Loading...',
        uniqueId: 'Loading...',
        deviceName: 'Loading...',
        model: 'Loading...',
        brand: 'Loading...',
    ...
    const fetchWindowsInfo = async () => {
        if (Platform.OS !== 'windows' || !RNDeviceInfo) {
            setWindowsInfo({
                diskCapacity: 'Not available on this platform',
                uniqueId: 'Not available on this platform',
                deviceName: 'Not available on this platform',
                model: 'Not available on this platform',
                brand: 'Not available on this platform',
            });
            return;
        }

        try {
            const [diskCapacity, uniqueId, deviceName, model, brand] = await Promise.all([
                RNDeviceInfo.getTotalDiskCapacity().then(result => `${(result / (1024 ** 3)).toFixed(2)} GB`).catch(() => 'Error'),
                RNDeviceInfo.getUniqueId().catch(() => 'Error'),
                RNDeviceInfo.getDeviceName().catch(() => 'Error'),
                RNDeviceInfo.getModel().catch(() => 'Error'),
                RNDeviceInfo.getBrand().catch(() => 'Error'),
            ]);

            setWindowsInfo({ diskCapacity, uniqueId, deviceName, model, brand });
        } catch (error) {
            console.log('Error fetching Windows info:', error);
        }
    };
    ...
    const testWindowsAPI = async (apiName, apiCall) => {
        if (Platform.OS !== 'windows' || !RNDeviceInfo) {
            Alert.alert('Windows API Test', 'This feature is only available on Windows platform');
            return;
        }
        ...
        try {
            ...
            if (apiName === 'Total Disk Capacity') {
                displayResult = `${(result / (1024 ** 3)).toFixed(2)} GB`;
            }

            Alert.alert(apiName, String(displayResult));
        } catch (error) {
            Alert.alert('Error', `Failed to get ${apiName}: ${error.message}`);
        }
    };

    return (
        <ScrollView style={styles.windowsContainer}>
            <Text style={styles.sectionTitle}>Windows-Specific APIs</Text>
            <Text style={styles.platformNote}>
                {Platform.OS === 'windows'
                    ? 'Running on Windows - APIs available'
                    : `Running on ${Platform.OS} - Windows APIs not available`}
            </Text>

            <View style={styles.infoSection}>
                <Text style={styles.infoLabel}>Total Disk Capacity:</Text>
                <Text style={styles.infoValue}>{windowsInfo.diskCapacity}</Text>
                <TouchableOpacity
                    style={styles.testButton}
                    onPress={() => testWindowsAPI('Total Disk Capacity', () => RNDeviceInfo?.getTotalDiskCapacity())}>
                    <Text style={styles.buttonText}>Test Disk Capacity</Text>
                </TouchableOpacity>
            </View>

            <View style={styles.infoSection}>
                <Text style={styles.infoLabel}>Unique ID:</Text>
                <Text style={styles.infoValue}>{windowsInfo.uniqueId}</Text>
                <TouchableOpacity
                    style={styles.testButton}
                    onPress={() => testWindowsAPI('Unique ID', () => RNDeviceInfo?.getUniqueId())}>
                    <Text style={styles.buttonText}>Test Unique ID</Text>
                </TouchableOpacity>
            </View>

            <View style={styles.infoSection}>
                <Text style={styles.infoLabel}>Device Name:</Text>
                <Text style={styles.infoValue}>{windowsInfo.deviceName}</Text>
                <TouchableOpacity
                    style={styles.testButton}
                    onPress={() => testWindowsAPI('Device Name', () => RNDeviceInfo?.getDeviceName())}>
                    <Text style={styles.buttonText}>Test Device Name</Text>
                </TouchableOpacity>
            </View>

            <View style={styles.infoSection}>
                <Text style={styles.infoLabel}>Model:</Text>
                <Text style={styles.infoValue}>{windowsInfo.model}</Text>
                <TouchableOpacity
                    style={styles.testButton}
                    onPress={() => testWindowsAPI('Model', () => RNDeviceInfo?.getModel())}>
            ...
            <View style={styles.infoSection}>
                ...
                <Text style={styles.infoValue}>{windowsInfo.brand}</Text>
                <TouchableOpacity
                    style={styles.testButton}
                    onPress={() => testWindowsAPI('Brand', () => RNDeviceInfo?.getBrand())}>
                    <Text style={styles.buttonText}>Test Brand</Text>
                </TouchableOpacity>
            </View>

            <TouchableOpacity
                style={[styles.testButton, styles.refreshButton]}
                onPress={fetchWindowsInfo}>
                <Text style={styles.buttonText}>Refresh All Windows Info</Text>
            </TouchableOpacity>
        </ScrollView>
    );
};
...
class App extends Component {
    ...
    getConstantDeviceInfo() {
        ...
        try {
            if (!DeviceInfo) {
                return { error: 'DeviceInfo not available' };
            }

            deviceJSON.deviceId = DeviceInfo.getDeviceId ? DeviceInfo.getDeviceId() : 'N/A';
            deviceJSON.bundleId = DeviceInfo.getBundleId ? DeviceInfo.getBundleId() : 'N/A';
            deviceJSON.systemName = DeviceInfo.getSystemName ? DeviceInfo.getSystemName() : 'N/A';
            deviceJSON.systemVersion = DeviceInfo.getSystemVersion ? DeviceInfo.getSystemVersion() : 'N/A';
            deviceJSON.version = DeviceInfo.getVersion ? DeviceInfo.getVersion() : 'N/A';
            deviceJSON.readableVersion = DeviceInfo.getReadableVersion ? DeviceInfo.getReadableVersion() : 'N/A';
            deviceJSON.buildNumber = DeviceInfo.getBuildNumber ? DeviceInfo.getBuildNumber() : 'N/A';
            deviceJSON.isTablet = DeviceInfo.isTablet ? DeviceInfo.isTablet() : false;
            deviceJSON.isLowRamDevice = DeviceInfo.isLowRamDevice ? DeviceInfo.isLowRamDevice() : false;
            deviceJSON.isDisplayZoomed = DeviceInfo.isDisplayZoomed ? DeviceInfo.isDisplayZoomed() : false;
            deviceJSON.appName = DeviceInfo.getApplicationName ? DeviceInfo.getApplicationName() : 'N/A';
            deviceJSON.brand = DeviceInfo.getBrand ? DeviceInfo.getBrand() : 'N/A';
            deviceJSON.model = DeviceInfo.getModel ? DeviceInfo.getModel() : 'N/A';
            deviceJSON.deviceType = DeviceInfo.getDeviceType ? DeviceInfo.getDeviceType() : 'N/A';
        } catch (error) {
            console.log('Error getting constant device info:', error);
            deviceJSON.error = 'Failed to get device info';
        }
    ...
    getSyncDeviceInfo() {
        ...
        try {
            ...
            console.log('Error getting sync device info:', error);
            deviceJSON.error = 'Failed to get sync device info';
        }
    ...
    render() {
        ...
        return (
            <SafeAreaView style={styles.container}>
                ...
                ) : this.state.activeTab === 'windows' ? (
                    <>
                        <Text style={styles.welcome}>
                            react-native-device-info example - Windows APIs:
                        </Text>
                ...
                <View style={styles.tabBar}>
                    ...
                    <TouchableOpacity
                        style={styles.tab}
                        testID="windows button"
                        onPress={() => this.setState({ activeTab: 'windows' })}>
                        <Text
                            style={[
                                styles.tabText,
                                this.state.activeTab === 'windows' && styles.boldText,
                            ]}>
                            Windows
                        </Text>
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    platformNote: {
        fontSize: 14,
        color: Platform.OS === 'windows' ? '#2e8b57' : '#d2691e',
        marginBottom: 20,
        fontStyle: 'italic',
    },
    infoSection: {
