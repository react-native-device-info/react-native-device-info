...
- (NSString *)getBuildId {
  NSString *buildId = @"unknown"; // Default value
  ...
  if (status == 0) {
    buildId = [[NSString alloc] initWithBytes:buffer.mutableBytes length:bufferSize encoding:NSUTF8StringEncoding];
  }
#endif
...
{
  return std::make_shared<facebook::react::NativeRNDeviceInfoIOSSpecJSI>(params);
}
#endif
